...
const engineNames: Record<string, string> = { bing: "Bing", baidu: "Baidu", duckduckgo: "DuckDuckGo", exa: "Exa", csdn: "CSDN", juejin: "Juejin", sogou: "Sogou" };
...
export const api = {
  ...
  history: async (): Promise<SearchHistory[]> => (await request<{ history: Array<{ id: number; query: string; engines: string[]; resultCount: number; snapshot: { results: Array<{ title: string; url: string; description: string; faviconUrl?: string; engines: string[] }>; failures: Array<{ engine: string; code: string; message: string }>; engineResults?: Array<{ engine: string; limit: number; results: Array<{ title: string; url: string; description: string; faviconUrl?: string; engines: string[] }>; cached: boolean; failure?: { engine: string; code: string; message: string } }> } | null; createdAt: string }> }>("/api/history")).history.map((item) => { const mapResults = (results: Array<{ title: string; url: string; description: string; faviconUrl?: string; engines: string[] }>, prefix: string) => results.map((result, index) => ({ id: `${prefix}-${index}`, ...result })); const engineResults: SearchEngineResultGroup[] = (item.snapshot?.engineResults ?? []).map((group) => ({ engine: group.engine, limit: group.limit, cached: group.cached, failure: group.failure, results: mapResults(group.results, `${item.id}-${group.engine}`) })); return { id: String(item.id), query: item.query, engines: item.engines, count: item.resultCount, results: mapResults(item.snapshot?.results ?? [], String(item.id)), engineResults, failures: item.snapshot?.failures ?? [], createdAt: item.createdAt }; }),
