...
function operationLabel(operation: string, english: boolean) {
  if (operation === "search") return english ? "Search" : "搜索";
  if (operation === "fetchWebContent") return english ? "Fetch web content" : "抓取网页正文";
  return operation;
}
...
export function UsagePage() {
  ...
  const text = english
    ? { title: "Usage statistics and audit logs", subtitle: "Review traffic, reliability, and engine statistics across a selected time range.", range: "Time range", custom: "Custom range", apply: "Apply range", calls: "Total requests", success: "Success rate", errors: "Errors", cache: "Cache hit rate", latency: "Avg latency", p95: "P95 latency", results: "Avg results", trend: "Request trend", trendDesc: "Requests grouped by selected range", status: "Outcome mix", channel: "Traffic split", operations: "Operation mix", enginesTitle: "Engine statistics", logs: "Request audit logs", allChannels: "All channels", allOperations: "All operations", allStatuses: "All statuses", allEngines: "All engines", clear: "Reset filters", previous: "Previous", next: "Next", noLogs: "No logs match the selected filters.", noData: "No audit data in this time range.", records: "records", web: "Web", mcp: "MCP", successes: "successful", partial: "partial", error: "errors", avg: "avg", today: "selected range", requestId: "Request ID", channelLabel: "Channel", operationLabel: "Operation", token: "Token", enginesLabel: "Engines", latencyLabel: "Latency", cacheLabel: "Cache", resultsLabel: "Results", statusLabel: "Status", errorLabel: "Error", createdAt: "Created at", hit: "HIT", miss: "MISS" }
    : { title: "调用统计与审计日志", subtitle: "按时间范围查看流量、稳定性和搜索引擎统计。", range: "统计范围", custom: "自定义范围", apply: "应用范围", calls: "总调用量", success: "成功率", errors: "错误请求", cache: "缓存命中率", latency: "平均延迟", p95: "P95 延迟", results: "平均结果数", trend: "调用趋势", trendDesc: "按当前时间范围聚合", status: "结果状态", channel: "流量来源", operations: "操作类型", enginesTitle: "引擎统计", logs: "请求审计日志", allChannels: "全部来源", allOperations: "全部操作", allStatuses: "全部状态", allEngines: "全部引擎", clear: "重置筛选", previous: "上一页", next: "下一页", noLogs: "当前筛选条件下没有日志。", noData: "当前统计范围内暂无审计数据。", records: "条记录", web: "网页", mcp: "MCP", successes: "成功", partial: "部分成功", error: "错误", avg: "平均", today: "当前范围", requestId: "请求 ID", channelLabel: "来源", operationLabel: "操作", token: "Token", enginesLabel: "引擎", latencyLabel: "延迟", cacheLabel: "缓存", resultsLabel: "结果数", statusLabel: "状态", errorLabel: "错误", createdAt: "创建时间", hit: "命中", miss: "未命中" };
  ...
  return (
    <section className="usage-page mx-auto w-full max-w-none space-y-5 pb-16">
      ...
      <div className="grid gap-5 lg:grid-cols-[1.45fr_0.95fr]">
        ...
        <Card className="usage-panel rounded-2xl border-slate-200 bg-white p-5"><div className="flex items-start justify-between"><div><h2 className="text-sm font-bold text-slate-900">{text.status}</h2><p className="mt-1 text-[10px] text-slate-400">{text.successes} / {text.partial} / {text.error}</p></div><Icon name="chart-donut" className="text-xl text-blue-500" /></div><div className="mt-6 flex h-3 overflow-hidden rounded-full bg-slate-100"><div className="bg-emerald-500" style={{ width: `${summary.total ? (summary.success / summary.total) * 100 : 0}%` }} /><div className="bg-amber-400" style={{ width: `${summary.total ? (summary.partial / summary.total) * 100 : 0}%` }} /><div className="bg-red-500" style={{ width: `${summary.total ? (summary.errors / summary.total) * 100 : 0}%` }} /></div><div className="mt-5 grid grid-cols-3 gap-2"><StatusStat label={text.successes} count={summary.success} total={summary.total} color="text-emerald-700" /><StatusStat label={text.partial} count={summary.partial} total={summary.total} color="text-amber-700" /><StatusStat label={text.error} count={summary.errors} total={summary.total} color="text-red-700" /></div><div className="mt-5 flex items-center justify-between border-t border-slate-100 pt-3 text-[10px] text-slate-500"><span>{text.channel}</span><span className="font-mono font-bold text-blue-700">{text.web} {webShare}% · {text.mcp} {mcpShare}%</span></div></Card>
      </div>
      ...
      <Card className="usage-panel overflow-hidden rounded-2xl border-slate-200 bg-white"><div className="audit-table-scroll table-scroll overflow-x-auto"><div className="min-w-[1100px]"><div className="space-y-3 border-b border-slate-100 bg-slate-50/60 p-4 sm:p-5"><div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between"><div><div className="flex items-center gap-2"><h2 className="text-sm font-bold text-slate-900">{text.logs}</h2><span className="rounded-lg bg-blue-50 px-2 py-1 font-mono text-[10px] font-bold text-blue-700">{pageSummary}</span></div><p className="mt-1 text-[10px] text-slate-400">{english ? "Server-side filters for the selected time range" : "服务端按当前统计范围筛选并分页"}</p></div><Button variant="ghost" className="min-h-8 self-start rounded-lg px-2.5 text-[11px]" onClick={clearFilters}><Icon name="funnel-simple" />{text.clear}</Button></div><div className="grid grid-cols-2 gap-2 sm:grid-cols-4"><Dropdown containerClassName="w-full" className="min-h-8 rounded-lg bg-white px-2.5 py-1.5 text-[11px] font-normal" value={channel} onChange={(value) => { setChannel(value); setPage(1); }} ariaLabel={text.allChannels} options={[{ value: "all", label: text.allChannels }, { value: "web", label: text.web }, { value: "mcp", label: text.mcp }]} /><Dropdown containerClassName="w-full" className="min-h-8 rounded-lg bg-white px-2.5 py-1.5 text-[11px] font-normal" value={operation} onChange={(value) => { setOperation(value); setPage(1); }} ariaLabel={text.allOperations} options={[{ value: "all", label: text.allOperations }, ...(data?.facets.operations ?? []).map((item) => ({ value: item, label: operationLabel(item, english) }))]} /><Dropdown containerClassName="w-full" className="min-h-8 rounded-lg bg-white px-2.5 py-1.5 text-[11px] font-normal" value={status} onChange={(value) => { setStatus(value); setPage(1); }} ariaLabel={text.allStatuses} options={[{ value: "all", label: text.allStatuses }, { value: "success", label: text.successes }, { value: "partial", label: text.partial }, { value: "error", label: text.error }]} /><Dropdown containerClassName="w-full" className="min-h-8 rounded-lg bg-white px-2.5 py-1.5 text-[11px] font-normal" value={engine} onChange={(value) => { setEngine(value); setPage(1); }} ariaLabel={text.allEngines} options={[{ value: "all", label: text.allEngines }, ...(data?.facets.engines ?? []).map((item) => ({ value: item, label: item }))]} /></div></div><div className="overflow-visible"><table className="w-full border-collapse text-left"><thead><tr className="border-b border-slate-200 bg-slate-100/70 text-[10px] font-bold uppercase tracking-wider text-slate-500"><th className="p-3 pl-5">{text.requestId}</th><th className="p-3">{text.channelLabel}</th><th className="p-3">{text.operationLabel}</th><th className="p-3">{text.token}</th><th className="p-3">{text.enginesLabel}</th><th className="p-3 text-right">{text.latencyLabel}</th><th className="p-3 text-right">{text.cacheLabel}</th><th className="p-3 text-right">{text.resultsLabel}</th><th className="p-3">{text.statusLabel}</th><th className="p-3">{text.errorLabel}</th><th className="p-3 pr-5">{text.createdAt}</th></tr></thead><tbody className="divide-y divide-slate-100 font-mono text-xs text-slate-700">{data?.logs.length ? data.logs.map((log) => <tr key={log.id} className="transition-colors hover:bg-blue-50/40"><td className="max-w-44 truncate p-3 pl-5 text-slate-500" title={log.id}>{log.id}</td><td className={`p-3 font-bold ${log.channel === "MCP" ? "text-blue-600" : "text-slate-800"}`}>{log.channel === "MCP" ? text.mcp : text.web}</td><td className="p-3 font-bold text-slate-800">{operationLabel(log.operation, english)}</td><td className="p-3 text-[11px] text-slate-400">{log.token}</td><td className="max-w-52 truncate p-3 text-slate-500">{log.engines.join(", ") || "—"}</td><td className="p-3 text-right tabular-nums text-slate-600">{log.latency}ms</td><td className="p-3 text-right"><span className={`rounded-md px-1.5 py-0.5 text-[10px] font-bold ${log.cacheHit ? "bg-blue-50 text-blue-700" : "bg-slate-100 text-slate-500"}`}>{log.cacheHit ? text.hit : text.miss}</span></td><td className="p-3 text-right font-bold tabular-nums text-slate-700">{log.resultCount}</td><td className="p-3"><span className={`rounded-md border px-1.5 py-0.5 text-[10px] font-bold ${statusClass(log.status)}`}>{log.status === "Success" ? text.successes : log.status === "Partial" ? text.partial : text.error}</span></td><td className={`max-w-48 truncate p-3 text-[10px] ${log.status === "Error" ? "text-red-600" : "text-slate-400"}`} title={log.errorCode}>{log.errorCode}</td><td className="whitespace-nowrap p-3 pr-5 text-slate-500">{shortTime(log.createdAt, english ? "en-US" : "zh-CN")}</td></tr>) : <tr><td colSpan={11}><EmptyState label={data ? text.noLogs : text.noData} /></td></tr>}</tbody></table></div><footer className="flex items-center justify-end gap-3 border-t border-slate-100 bg-slate-50/50 px-4 py-3"><span className="font-mono text-[10px] text-slate-400">{currentPage} / {totalPages}</span><Button variant="secondary" className="min-h-8 rounded-lg px-3 py-1.5 text-[11px]" disabled={currentPage <= 1 || loading} onClick={() => setPage((value) => Math.max(1, value - 1))}>{text.previous}</Button><Button variant="secondary" className="min-h-8 rounded-lg px-3 py-1.5 text-[11px]" disabled={currentPage >= totalPages || loading} onClick={() => setPage((value) => Math.min(totalPages, value + 1))}>{text.next}</Button></footer></div></div></Card>
    </section>
  );
}
