...
const resultLimitValues = (engine: SearchEngine): number[] => {
  const effective = engine.resultLimit === null ? undefined : Math.min(engine.resultLimit, engine.maxResults);
  return [...new Set([5, 10, 20, 30, 50, ...(effective === undefined ? [] : [effective])])]
    .filter((value) => value <= engine.maxResults)
    .sort((a, b) => a - b);
};
...
export function EnginesPage() {
  ...
  const saveCredential = async () => {
    ...
    if (!value) return notify(english ? "Enter a credential, or use Clear to remove the saved credential." : "请输入凭据；如需删除已保存凭据，请点击清除。", "error");
    ...
    try {
      ...
      const restartRequired = credentialEngine.id === "exa";
      notify(restartRequired
        ? (english ? "API Key saved. Restart the Open-WebSearch daemon for it to take effect." : "API Key 已保存；请重启 Open-WebSearch daemon 后生效。")
        : (english ? "Credential saved and active immediately." : "凭据已保存，立即生效。"), "info");
    } catch (error) {
      const detail = error instanceof Error ? error.message : (english ? "Could not save credential." : "凭据保存失败。");
      notify(friendlyError({ code: error instanceof ApiClientError ? error.code : undefined, message: detail }, english).summary, "error");
  ...
  const clearCredential = async () => {
    ...
    try {
      ...
      const restartRequired = credentialEngine.id === "exa";
      notify(restartRequired
        ? (english ? "Credential cleared. Restart the Open-WebSearch daemon if it is still running with the old key." : "凭据已清除；如 daemon 仍在运行，请重启以移除旧密钥。")
        : (english ? "Credential cleared and inactive immediately." : "凭据已清除，立即失效。"), "info");
    } catch (error) {
      const detail = error instanceof Error ? error.message : (english ? "Could not clear credential." : "凭据清除失败。");
      notify(friendlyError({ code: error instanceof ApiClientError ? error.code : undefined, message: detail }, english).summary, "error");
    } finally { setSavingCredential(false); }
  };
  ...
  return <section className="mx-auto w-full max-w-none space-y-5 pb-16">
    ...
    <div className="white-card overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-xs"><div className="table-scroll overflow-x-auto"><table className="w-full min-w-[1120px] border-collapse text-left"><thead><tr className="border-b border-slate-200 bg-slate-100/70 text-[11px] font-bold uppercase tracking-wider text-slate-500"><th className="whitespace-nowrap p-4">{english ? "Engine" : "搜索引擎"}</th><th className="whitespace-nowrap p-4 text-center">{english ? "Enabled" : "启用 (Enabled)"}</th><th className="whitespace-nowrap p-4 text-center">{english ? "Default" : "默认选定"}</th><th className="whitespace-nowrap p-4">Search Mode</th><th className="whitespace-nowrap p-4">{t.resultLimit}</th><th className="whitespace-nowrap p-4">{english ? "Prerequisites" : "启用条件"}</th><th className="whitespace-nowrap p-4">{english ? "Last test" : "最近测试"}</th><th className="whitespace-nowrap p-4">{english ? "Health" : "健康度状态"}</th><th className="whitespace-nowrap p-4">{english ? "Latency" : "延迟"}</th><th className="whitespace-nowrap p-4">{english ? "Last error" : "最近错误"}</th><th className="whitespace-nowrap p-4 text-right">{english ? "Action" : "操作"}</th></tr></thead><tbody className="divide-y divide-slate-100 font-mono text-xs">{engines.map((engine) => <tr key={engine.id} className="hover:bg-slate-50/50"><td className="p-4 font-sans"><EngineTag engine={engine.name} compact /></td><td className="p-4 text-center"><Switch label={`${engine.name} ${t.enabled}`} checked={engine.enabled} onChange={(value) => void toggleEnabled(engine, value)} /></td><td className="p-4 text-center"><input type="checkbox" aria-label={`${engine.name} ${t.default}`} checked={engine.isDefault} onChange={(event) => void patch(engine.id, { isDefault: event.target.checked })} className="rounded border-slate-300 text-blue-600 focus:ring-blue-500" /></td><td className="whitespace-nowrap p-4 font-bold text-slate-600">{engine.mode}</td><td className="min-w-40 p-3"><Dropdown placement="bottom" containerClassName="w-40" className="w-full rounded-lg bg-white py-1 text-[11px]" value={engine.resultLimit === null ? "default" : String(Math.min(engine.resultLimit, engine.maxResults))} onChange={(value) => void patch(engine.id, { resultLimit: value === "default" ? null : Number(value) })} ariaLabel={`${engine.name} ${t.resultLimit}`} options={[{ value: "default", label: t.systemDefault }, ...resultLimitValues(engine).map((value) => ({ value: String(value), label: `${value} ${english ? "results" : "条"}` }))]} /></td><td className="p-4 font-sans"><div className="flex flex-wrap items-center gap-1.5">{engine.requiresProxy ? <span className="rounded-full border border-sky-200 bg-sky-50 px-2 py-0.5 text-[10px] font-bold text-sky-700"><Icon name="globe" className="mr-1" />Proxy</span> : null}{engine.supportsApiKey ? <button type="button" title={english ? "Click to edit credential" : "点击修改凭据"} aria-label={`${engine.name} ${english ? "credential; click to edit" : "凭据，点击修改"}`} onClick={() => openCredential(engine)} className={`rounded-full border px-2 py-0.5 text-[10px] font-bold ${engine.apiKeyConfigured ? "border-emerald-200 bg-emerald-50 text-emerald-700" : "border-amber-200 bg-amber-50 text-amber-800"}`}><Icon name="key" className="mr-1" />{engine.apiKeyConfigured ? (english ? "Credential ready" : "已配置凭据") : engine.apiKeyOptional ? (english ? "Token optional" : "Token 可选") : (english ? "Key needed" : "需要 Key")}<span className="ml-1 font-normal opacity-70">· {english ? "click to edit" : "点击修改"}</span></button> : engine.requiresProxy ? null : <span className="text-[10px] text-slate-400">{english ? "None" : "无"}</span>}</div></td><td className="whitespace-nowrap p-4 text-slate-500">{displayTime(engine.lastTestAt, english ? "en-US" : "zh-CN")}</td><td className="whitespace-nowrap p-4"><Tag tone={healthTone[engine.health]}>{healthLabel(engine.health, english)}</Tag></td><td className="whitespace-nowrap p-4 font-bold text-blue-600">{engine.latency === null ? "—" : `${engine.latency}ms`}</td><td className="p-4 font-sans text-xs text-slate-400">{engine.lastError === "—" ? "—" : <ErrorDisclosure message={engine.lastError} english={english} />}</td><td className="whitespace-nowrap p-4 text-right"><button type="button" className="text-xs font-bold text-blue-600 hover:underline" onClick={() => openTest(engine)}>{t.testSearch}</button></td></tr>)}</tbody></table></div></div>
    ...
    {credentialEngine ? <Modal title={`${t.configure} · ${credentialEngine.name}`} onClose={() => { if (!savingCredential) setCredentialEngine(null); }} closeLabel={english ? "Close credential dialog" : "关闭凭据配置弹窗"} footer={<div className="flex w-full flex-wrap items-center justify-between gap-2"><Button variant="ghost" className="text-red-600 hover:bg-red-50" disabled={savingCredential || !credentialEngine.apiKeyConfigured} onClick={() => void clearCredential()}><Icon name="trash" />{english ? "Clear saved credential" : "清除已保存凭据"}</Button><div className="flex gap-2"><Button variant="secondary" disabled={savingCredential} onClick={() => setCredentialEngine(null)}>{english ? "Cancel" : "取消"}</Button><Button disabled={savingCredential} onClick={() => void saveCredential()}>{savingCredential ? "…" : (english ? "Save credential" : "保存凭据")}</Button></div></div>}><div className="space-y-4"><label className="block text-xs font-bold text-slate-700"><span>{credentialEngine.credentialLabel ?? (english ? "API Key" : "API Key")}</span><span className="relative mt-1 block"><Input autoFocus className="pr-11 font-mono" type={credentialVisible ? "text" : "password"} placeholder={credentialEngine.apiKeyConfigured ? (english ? "Enter a new credential to replace the saved one" : "输入新凭据以替换已保存值") : (credentialEngine.credentialPlaceholder ?? (english ? "Paste the credential" : "粘贴凭据"))} value={credentialDraft} onChange={(event) => setCredentialDraft(event.target.value)} onKeyDown={(event) => { if (event.key === "Enter") void saveCredential(); }} /><button type="button" className="absolute right-1 top-1/2 grid size-8 -translate-y-1/2 place-items-center rounded-lg text-slate-400 hover:bg-slate-100 hover:text-blue-600" aria-label={credentialVisible ? (english ? "Hide credential" : "隐藏凭据") : (english ? "Show credential" : "显示凭据")} onClick={() => setCredentialVisible((value) => !value)}><Icon name={credentialVisible ? "eye-slash" : "eye"} /></button></span></label>{credentialEngine.credentialUrl ? <a href={credentialEngine.credentialUrl} target="_blank" rel="noreferrer" className="flex items-center gap-2 rounded-xl border border-blue-200 bg-blue-50 px-3 py-2.5 text-xs font-bold text-blue-700 underline decoration-blue-300 underline-offset-2 transition-colors hover:border-blue-400 hover:bg-blue-100 hover:text-blue-800"><Icon name="arrow-square-out" className="shrink-0" />{english ? "Open the official page to apply for a key/token" : "前往官网申请 API Key / Token"}<span className="ml-auto truncate font-mono text-[10px] font-semibold no-underline opacity-80">{credentialEngine.credentialUrl}</span></a> : null}{credentialEngine.apiKeyOptional ? <p className="text-xs leading-5 text-slate-500">{english ? "This credential is optional. GitHub public repository search also works anonymously, with lower rate limits." : "该凭据为可选项。GitHub 公共仓库搜索支持匿名访问，但匿名限流更严格。"}</p> : null}{enableAfterSave ? <p className="text-xs font-semibold text-blue-700">{english ? "After saving, this engine will be enabled automatically." : "保存后会自动尝试启用此引擎。"}</p> : null}</div></Modal> : null}
  </section>;
}
